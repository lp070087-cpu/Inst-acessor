...
const signals_1 = require("../src/lib/growth-engine/signals");
const priorities_1 = require("../src/lib/growth-engine/priorities");
const recommendations_1 = require("../src/lib/growth-engine/recommendations");
const insights_1 = require("../src/lib/growth-engine/insights");
const progress_1 = require("../src/lib/growth-engine/progress");
const automations_1 = require("../src/lib/growth-engine/automations");
const signature_1 = require("../src/lib/webhooks/signature");
// ------------------------------------------------------------
// Fixtures
// ------------------------------------------------------------
...
async function main() {
    ...
    console.log("\n[1] Detecção de sinais");
    {
        ...
        const signals = await (0, signals_1.detectSignals)(ctx);
    ...
    // 4) Priorização: máx 3 + dedup por tipo
    {
        ...
        const signals = await (0, signals_1.detectSignals)(ctx);
        const priorities = (0, priorities_1.prioritizeSignals)(ctx, signals);
    ...
    // 5) Recomendações acionáveis
    {
        ...
        const signals = await (0, signals_1.detectSignals)(ctx);
        const priorities = (0, priorities_1.prioritizeSignals)(ctx, signals);
        const recs = (0, recommendations_1.buildRecommendations)(ctx, priorities);
    ...
    // 11) Insights proativos (nunca inventa)
    {
        const ctx = makeCtx({});
        const signals = await (0, signals_1.detectSignals)(ctx);
        const insights = (0, insights_1.buildProactiveInsights)(signals);
    ...
    // 12) Plano 7 dias
    {
        ...
        const priorities = (0, priorities_1.prioritizeSignals)(ctx, signals);
        const recs = (0, recommendations_1.buildRecommendations)(ctx, priorities);
        const plan = (0, progress_1.buildPlan7Days)(ctx, priorities, recs);
    ...
    // 13) Plano 30 dias adapta ao estágio
    {
        ...
        const plan = (0, progress_1.buildPlan30Days)(ctxSemDados);
    ...
    {
        const ctx = makeCtx({});
        const mission = (0, progress_1.pickDailyMission)(ctx, [], [], []);
    ...
    // 15) Automações internas (sem ação externa)
    {
        ...
        const priorities = (0, priorities_1.prioritizeSignals)(ctx, signals);
        const recs = (0, recommendations_1.buildRecommendations)(ctx, priorities);
        const automations = (0, automations_1.buildInternalAutomations)(signals, recs);
    ...
    // 18) Verificação de assinatura de webhook (X-Hub-Signature-256)
    {
        ...
        const signature = `sha256=${(0, node_crypto_1.createHmac)("sha256", secret).update(body, "utf8").digest("hex")}`;
        test("assinatura X-Hub-Signature-256 válida é aceita", () => {
            assert.strictEqual((0, signature_1.verifyWebhookSignature)(body, signature, secret), true);
        });
        test("assinatura com segredo errado é rejeitada", () => {
            assert.strictEqual((0, signature_1.verifyWebhookSignature)(body, signature, "outro_segredo"), false);
        });
        test("payload adulterado é rejeitado", () => {
            const tampered = JSON.stringify({ object: "instagram", entry: [1] });
            assert.strictEqual((0, signature_1.verifyWebhookSignature)(tampered, signature, secret), false);
        });
        test("ausência de assinatura é rejeitada", () => {
            assert.strictEqual((0, signature_1.verifyWebhookSignature)(body, null, secret), false);
        });
        test("formato sha256= é aceito e sem prefixo também", () => {
            const raw = (0, node_crypto_1.createHmac)("sha256", secret).update(body, "utf8").digest("hex");
            assert.strictEqual((0, signature_1.verifyWebhookSignature)(body, `sha256=${raw}`, secret), true);
            assert.strictEqual((0, signature_1.verifyWebhookSignature)(body, raw, secret), true);
        });
    }
