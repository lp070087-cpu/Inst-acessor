...
exports.pickDailyMission = pickDailyMission;
exports.buildPlan7Days = buildPlan7Days;
exports.buildPlan30Days = buildPlan30Days;
exports.iso = iso;
 ...
 * INSUFICIENTE.
 */
function pickDailyMission(ctx, priorities, recommendations, actions) {
...
// ------------------------------------------------------------
function planItem(type, action, recommendationSlug, platform) {
 ...
 */
function buildPlan30Days(ctx) {
    const stage = ctx.stage;
    const platform = ctx.instagram.connected ? "instagram" : "tiktok";
    const weeks = [];
    if (!ctx.instagram.connected && !ctx.tiktok.connected) {
        weeks.push({ week: 1, theme: "Diagnóstico", focus: "Conexão", items: ["Conectar Instagram ou TikTok", "Sincronizar métricas iniciais", "Definir objetivo no perfil"] }, { week: 2, theme: "Base", focus: "Perfil e conteúdo", items: ["Otimizar bio e destaques", "Selecionar 5 ideias de conteúdo", "Agendar primeiras 3 publicações"] }, { week: 3, theme: "Consistência", focus: "Publicação", items: ["Publicar 3 vezes na semana", "Responder comentários", "Revisar primeiras métricas"] }, { week: 4, theme: "Análise", focus: "Resultados", items: ["Calcular Score", "Analisar o que funcionou", "Ajustar estratégia para o mês seguinte"] });
    }
    else if (stage === "inicio") {
        weeks.push({ week: 1, theme: "Diagnóstico", focus: "Base", items: ["Sincronizar métricas", "Revisar diagnóstico", "Definir metas de 30 dias"] }, { week: 2, theme: "Execução", focus: "Conteúdo", items: ["Publicar 4-5 conteúdos", "Criar rotina de resposta", "Rodar 1 experimento simples"] }, { week: 3, theme: "Otimização", focus: "Perfil e formatos", items: ["Ajustar bio com CTA", "Testar 2 formatos diferentes", "Analisar melhores horários"] }, { week: 4, theme: "Escala", focus: "Resultados", items: ["Calcular Score", "Identificar o que funcionou", "Planejar o próximo mês"] });
    }
    else {
        weeks.push({ week: 1, theme: "Diagnóstico", focus: "Base", items: ["Sincronizar métricas e Score", "Revisar sinais e prioridades", "Atualizar metas"] }, { week: 2, theme: "Execução", focus: "Consistência", items: ["Manter frequência de 3+/semana", "Escalar conteúdo de alto desempenho", "Responder comunidade"] }, { week: 3, theme: "Otimização", focus: "Experimentos", items: ["Rodar experimentos com amostra suficiente", "Analisar resultados", "Ajustar formato com base em dados"] }, { week: 4, theme: "Escala", focus: "Análise", items: ["Analisar crescimento do mês", "Revisar Score e diagnóstico", "Definir próximo ciclo"] });
    }
    return {
        weeks,
        generatedAt: iso(new Date()),
