...
function metricDropSignals(ctx) {
    const out = [];
    for (const platform of ["instagram", "tiktok"]) {
        const p = ctx[platform];
        if (p.status !== "DADOS_SUFICIENTES")
            continue;
...
function postingSignals(ctx) {
    const out = [];
    for (const platform of ["instagram", "tiktok"]) {
        const p = ctx[platform];
        if (p.status === "SEM_REDE" || p.status === "SEM_SYNC")
            continue;
...
function goalSignals(ctx) {
    const out = [];
    const allGoals = [...ctx.instagram.goals, ...ctx.tiktok.goals].filter((g, i, arr) => arr.findIndex((x) => x.id === g.id) === i);
...
function experimentSignals(ctx) {
    const out = [];
    const experiments = [
...
function retentionSignals(ctx) {
    const out = [];
    for (const platform of ["instagram", "tiktok"]) {
        const p = ctx[platform];
        if (p.engagement == null || p.mediaCount == null || p.mediaCount === 0)
            continue;
...
function highPerformingSignals(ctx) {
    const out = [];
    for (const platform of ["instagram", "tiktok"]) {
        const p = ctx[platform];
        if (p.status !== "DADOS_SUFICIENTES")
 ...
 */
async function detectSignals(ctx) {
    const all = [
        ...accountSignals(ctx),
