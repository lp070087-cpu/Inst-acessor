...
exports.growthContextToPrompt = growthContextToPrompt;
/**
...
function platformBlock(ctx, p) {
    const lines = [];
    lines.push(`- ${p.platform === "instagram" ? "Instagram" : "TikTok"}: status = ${dataStatusLabel(p.status)}${p.connected ? "" : " (não conectada)"}`);
    if (p.followers != null)
        lines.push(`  - seguidores: ${fmtNumber(p.followers)}`);
    if (p.reach != null)
        lines.push(`  - alcance: ${fmtNumber(p.reach)}`);
    if (p.impressions != null)
        lines.push(`  - impressões: ${fmtNumber(p.impressions)}`);
    if (p.engagement != null)
        lines.push(`  - engajamento: ${fmtNumber(p.engagement)}`);
    if (p.growth != null)
        lines.push(`  - crescimento mensal: ${p.growth}%`);
    if (p.frequency != null)
        lines.push(`  - frequência estimada: ${p.frequency} posts/semana`);
    if (p.score != null)
        lines.push(`  - score: ${p.score} (cobertura ${fmtNumber(p.scoreCoverage)}%)`);
    if (p.goals.length > 0) {
        lines.push(`  - metas: ${p.goals.map((g) => `${g.title} (${g.progressPercent}%)`).join(", ")}`);
    }
    if (p.experiments.length > 0) {
        lines.push(`  - experimentos: ${p.experiments.map((e) => `${e.hypothesis} (${e.status})`).join("; ")}`);
    }
    if (p.alerts.length > 0) {
        lines.push(`  - alertas: ${p.alerts.map((a) => a.title).join("; ")}`);
    }
    if (p.plannedContent.length > 0) {
        lines.push(`  - conteúdos planejados: ${p.plannedContent.map((c) => c.title).join("; ")}`);
    }
    if (p.bestContent.length > 0) {
        lines.push(`  - melhor conteúdo: ${p.bestContent.map((c) => `${c.label} em ${c.date} (${c.value})`).join("; ")}`);
    }
    if (p.worstContent.length > 0) {
        lines.push(`  - pior conteúdo: ${p.worstContent.map((c) => `${c.label} em ${c.date} (${c.value})`).join("; ")}`);
    }
    return lines;
}
function growthContextToPrompt(ctx) {
    const lines = [];
    ...
    lines.push("");
    lines.push("Regra: esses campos são DADOS REAIS quando presentes. Ausência significa DADO INSUFICIENTE — responda 'DADO INSUFICIENTE' e não invente métricas.");
    return lines.join("\n");
}
