...
exports.SIGNAL_LABELS = void 0;
exports.prioritizeSignals = prioritizeSignals;
/**
...
function findGoalForSignal(ctx, signal) {
    ...
    if (category) {
        const match = goals.find((g) => g.category === category && g.status === "ATIVA");
        if (match)
            return match.title;
    }
    return null;
...
function prioritizeSignals(ctx, signals) {
    ...
    for (const item of scored) {
        if (picked.length >= 3)
            break;
        if (seenTypes.has(item.signal.type))
            continue;
    ...
    return picked.map((item, idx) => ({
        ...
        title: exports.SIGNAL_LABELS[item.signal.type],
