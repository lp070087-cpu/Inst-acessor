...
exports.buildGrowthContext = buildGrowthContext;
const db_1 = require("@/lib/db");
const instagram_data_1 = require("@/lib/dashboard/instagram-data");
const tiktok_data_1 = require("@/lib/dashboard/tiktok-data");
const services_1 = require("@/lib/ai/services");
const gamification_1 = require("@/lib/gamification");
const experiments_1 = require("@/lib/knowledge/experiments");
const patterns_1 = require("@/lib/knowledge/patterns");
const alerts_1 = require("@/lib/knowledge/alerts");
...
function deriveStage(followersIG, followersTT) {
...
async function loadPlatformData(userId, platform) {
...
async function buildPlatformContext(userId, platform) {
    ...
    if (!data.connected)
        ...
        status = "SEM_SYNC";
    else if (data.snapshotCount < 2)
        status = "POUCOS_DADOS";
    else
        status = "DADOS_SUFICIENTES";
    // Score real mais recente
    const score = await (0, services_1.getLatestScore)(userId, platform);
    // Metas reais da plataforma
    const goals = await (0, gamification_1.listGoals)(userId);
    const goalsForPlatform = goals.filter((g) => !g.platform || g.platform === platform);
    // Experimentos reais da plataforma
    const experiments = await (0, experiments_1.listExperiments)(userId);
    const experimentsForPlatform = experiments.filter((e) => !e.platform || e.platform === platform);
    // Alertas reais
    const alerts = await (0, alerts_1.generateAlerts)(userId, platform);
    // Conteúdos planejados
    const planned = await (0, planning_1.listPlannedContent)(userId, { platform });
 ...
 */
async function buildGrowthContext(userId) {
    const [profile, instagram, tiktok, insights, intelligence] = await Promise.all([
        db_1.prisma.userProfile.findUnique({ where: { userId } }),
        buildPlatformContext(userId, "instagram"),
        buildPlatformContext(userId, "tiktok"),
        Promise.all([(0, patterns_1.listInsights)(userId, "instagram"), (0, patterns_1.listInsights)(userId, "tiktok")]),
        (0, services_1.getAIProfile)(userId),
    ]);
    ...
    let confidence = 1.0;
    if (!(instagram.connected || tiktok.connected))
        confidence *= 0.5;
    if (!(instagram.snapshotCount >= 2 || tiktok.snapshotCount >= 2))
        confidence *= 0.5;
    if (!(instagram.score != null || tiktok.score != null))
        confidence *= 0.5;
