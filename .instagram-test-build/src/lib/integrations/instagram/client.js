...
exports.FETCH_TIMEOUT_MS = exports.INSTAGRAM_GRAPH_VERSION = exports.InstagramApiError = exports.INSTAGRAM_AUTHORIZE_BASE = exports.INSTAGRAM_OAUTH_BASE = exports.INSTAGRAM_GRAPH_BASE = void 0;
exports.getMetaCredentials = getMetaCredentials;
exports.getWebhookAppSecret = getWebhookAppSecret;
exports.getRedirectUri = getRedirectUri;
exports.graphGet = graphGet;
exports.graphGetNoRetry = graphGetNoRetry;
exports.postFormNoRetry = postFormNoRetry;
...
function getMetaCredentials() {
    ...
    if (!appId || !appSecret) {
        throw new errors_1.IntegrationConfigError("Instagram não configurado no servidor (INSTAGRAM_APP_ID/INSTAGRAM_APP_SECRET).");
    }
...
function getRedirectUri() {
    ...
    if (!redirectUri) {
        throw new errors_1.IntegrationConfigError("INSTAGRAM_REDIRECT_URI não configurada no servidor.");
    }
    return redirectUri;
}
/**
...
async function graphGet(path, accessToken) {
    const url = `${exports.INSTAGRAM_GRAPH_BASE}/${GRAPH_VERSION}/${path}${path.includes("?") ? "&" : "?"}access_token=${encodeURIComponent(accessToken)}`;
    let lastError;
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            if (transient && attempt < MAX_RETRIES) {
                lastError = new InstagramApiError(isAbort(error) ? "A API do Instagram demorou para responder." : "Falha de rede ao consultar a API do Instagram.", isAbort(error) ? "TIMEOUT" : "NETWORK", true);
                continue;
            }
            throw new InstagramApiError(isAbort(error) ? "A API do Instagram demorou para responder." : "Falha de rede ao consultar a API do Instagram.", isAbort(error) ? "TIMEOUT" : "NETWORK");
        }
    }
 ...
 */
async function graphGetNoRetry(url) {
    const res = await fetch(url, { method: "GET", signal: timeoutSignal() });
 ...
 */
async function postFormNoRetry(url, form) {
