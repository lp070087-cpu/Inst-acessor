...
exports.STREAK_BONUS_XP = exports.STREAK_BONUS_MILESTONES = exports.RITMO_PERIODS = exports.RITMO_KINDS = exports.RITMO_BANDS = void 0;
...
exports.baselineBefore = baselineBefore;
exports.inWindow = inWindow;
exports.followersDelta = followersDelta;
exports.growthPct = growthPct;
exports.combinedGrowthPct = combinedGrowthPct;
...
function weekKey(d) {
    ...
    const week = Math.ceil(((c.getTime() - yearStart.getTime()) / 864e5 + yearStart.getDay() + 1) / 7);
...
function unitForCount(title, n) {
    if (/seguidores/i.test(title))
        return "seguidores";
    if (/ideia/i.test(title))
        return n === 1 ? "ideia" : "ideias";
    if (/copy/i.test(title))
        return n === 1 ? "copy" : "copies";
    if (/IA|consulta/i.test(title))
        return n === 1 ? "consulta" : "consultas";
    if (/publica|conteúdo|conteudo/i.test(title))
        return n === 1 ? "publicação" : "publicações";
    return "";
}
exports.RITMO_BANDS = [
    // --- Diárias ---
...
exports.RITMO_KINDS = exports.RITMO_BANDS.map((b) => b.id);
exports.RITMO_PERIODS = ["dia", "semana", "mes"];
const PERIOD_LABEL = {
    dia: "hoje",
    semana: "na semana",
    mes: "no mês",
...
function deriveStreak(rows, today = new Date()) {
    if (rows.length === 0)
        return { streakDays: 0, activeWeekStreak: 0 };
    const days = new Set();
    for (const r of rows)
        days.add(fmtDay(r.createdAt));
    // Dias corridos (se hoje ainda não tem XP, conta a partir de ontem).
    let cursor = startOfDay(today);
    if (!days.has(fmtDay(cursor)))
    ...
    for (;;) {
        if ((activeWeek.get(wkCursor) ?? 0) < 3)
            break;
        activeWeekStreak += 1;
...
// ------------------------------------------------------------
exports.STREAK_BONUS_MILESTONES = [3, 7, 15, 30];
exports.STREAK_BONUS_XP = { 3: 20, 7: 50, 15: 120, 30: 300 };
// ------------------------------------------------------------
// Derivadores puros por janela
// ------------------------------------------------------------
/** Linha imediatamente anterior à janela (baseline), ou null. */
function baselineBefore(series, since) {
    let base = null;
    for (const s of series) {
        if (s.capturedAt.getTime() >= since.getTime())
            break;
        base = s;
    }
    return base;
}
function inWindow(series, since) {
 ...
 */
function followersDelta(g, since) {
    ...
    for (const series of platforms) {
        ...
        if (rows.length === 0)
            continue;
        measured = true;
        const last = rows[rows.length - 1].followersCount;
...
function growthPct(series, since, key) {
    ...
    const startRef = typeof base === "number" ? base : rows.length >= 2 ? rows[0] : null;
    if (startRef === null)
        return { value: 0, available: false };
    if (startRef <= 0) {
...
function combinedGrowthPct(g, since) {
    let latestTotal = 0;
    let baseTotal = null;
    let measured = false;
...
function countSince(dates, since) {
    return dates.filter((d) => d.getTime() >= since.getTime()).length;
}
function measureBand(g, band, since) {
    switch (band.metric) {
        case "seguidores":
...
function remainingLabel(period, until) {
    const ms = until.getTime() - Date.now();
    if (ms <= 0)
        return "Encerra agora";
    if (period === "dia") {
...
function buildCards(g, w, grantedSet) {
    const cards = [];
    for (const band of exports.RITMO_BANDS) {
...
function buildState(g, grantedSet, bonusesGranted) {
    const w = windows(g);
    ...
    const bonusXpGranted = bonusesGranted.reduce((s, d) => s + (exports.STREAK_BONUS_XP[d] ?? 0), 0);
    return {
        cards: buildCards(g, w, grantedSet),
        streakDays,
        activeWeekStreak,
        nextStreakBonus: exports.STREAK_BONUS_MILESTONES.find((m) => m > streakDays) ??
            exports.STREAK_BONUS_MILESTONES[exports.STREAK_BONUS_MILESTONES.length - 1],
        todayXp,
        bonusXpGranted,
    };
}
